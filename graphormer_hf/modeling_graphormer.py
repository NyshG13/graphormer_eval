...
class GraphormerGraphEncoderLayer(nn.Module):
    def __init__(self, config: GraphormerConfig) -> None:
        ...
        self.q_noise = config.q_noise  # quantization noise: REMOVED
        self.qn_block_size = config.qn_block_size  # quantization noise: REMOVED
        ...
        self.fc1 = nn.Linear(self.embedding_dim, config.ffn_embedding_dim)
        self.fc2 = nn.Linear(config.ffn_embedding_dim, self.embedding_dim)
...
class GraphormerDiffusion(nn.Module):
    def __init__(self, config: GraphormerConfig):
        super().__init__()
        self.config = config
        self.diffusion_model = GraphLatentDiffusion(
    ...
    def forward(self, input_nodes, edge_index, labels=None, node_mask=None, 
                aug_added_edges=None, aug_removed_edges=None, aug_original_edges=None,
                log_group=None, log_step=None):
        ...
        node_emb, attention_matching_loss = self.diffusion_model(
            node_emb, edge_index,
            aug_added_edges=aug_added_edges,
            aug_removed_edges=aug_removed_edges,
...
class GraphormerForNodeClassification(GraphormerPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        attention_matching_loss = torch.tensor(0.0, device=input_nodes.device)
        if self.config.enable_diffusion and self.diffusion is not None:
            outputs, attention_matching_loss = self.diffusion(
                input_nodes, edge_index, labels=labels, node_mask=node_mask,
        ...
        if loss is not None:
            ...
            if log_step_value is not None:
                log_payload["step"] = log_step_value
            # Remove Nones to avoid wandb errors
