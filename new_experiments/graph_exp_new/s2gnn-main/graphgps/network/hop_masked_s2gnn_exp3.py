"""Hop-Masked S2GNN — Experiment 3: Multi-Feature Structural Bias.

Registered as ``hop_masked_s2gnn_exp3``.

Changes vs baseline
-------------------
- Uses MultiFeatureBiasTransformerLayer (Exp 1 hop bias + sigma + cnbr).
- Loads path_features from batch (sigma, cnbr pre-padded via pad_path_features).
...
from graphgps.layer.hop_masked_transformer_exp3 import MultiFeatureBiasTransformerLayer
...
class BatchHopMaskedS2GNNLayerExp3(BatchHopMaskedS2GNNLayer):
    """BatchHopMaskedS2GNNLayer with multi-feature structural bias (Exp 3)."""
    ...
    def forward(self, batch):
        ...
        # Pad path features (sigma, cnbr already log1p'd in pad_path_features)
        pf = pad_path_features(batch, N_max, self.max_hops, dense_x.device)
        sigma = pf['sigma']  # (B, N_max, N_max) float
        cnbr  = pf['cnbr']  # (B, N_max, N_max) float

        if cfg.gnn.spectral.combine_with_spatial is None:
            # ---- SEQUENTIAL ----
            h = dense_x
            for t_layer in self.transformer_layers:
                h = t_layer(h, per_head_mask, mask, dist_int, sigma, cnbr)
...
@register_network('hop_masked_s2gnn_exp3')
class HopMaskedS2GNNExp3(nn.Module):
    """S2GNN with Multi-Feature Structural Bias — Experiment 3."""
    ...
    def __init__(self, dim_in, dim_out):
        ...
        for i in range(cfg.gnn.layers_mp):
            ...
            tf_layers = nn.ModuleList([
                MultiFeatureBiasTransformerLayer(hidden_dim=hidden_dim,
                                                  num_heads=num_heads,
                                                  ffn_dim=ffn_dim,
                                                  max_hops=num_hops,
                                                  dropout=dropout,
            ...
            layers.append(BatchHopMaskedS2GNNLayerExp3(
